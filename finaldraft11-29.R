...
# need help here: recursive indexing error 
...
tree <- rpart(Target ~ ., data=data1, method="class", 
...
# AI below is also giving same error 
...
bagging_pred_numeric <- matrix(0, nrow = nrow(test_data), ncol = B)
n_train <- nrow(train_data)

for (b in 1:B) {
  boot_indices <- sample(n_train, replace = TRUE)
  stump <- rpart(y ~ ., data = train_data[boot_indices,], control = rpart.control(maxdepth = 1))
  preds_factor <- predict(stump, newdata = test_data, type = "class")
  bagging_pred_numeric[, b] <- as.numeric(as.character(preds_factor)) 
}

test_y_numeric <- as.numeric(as.character(test_data$y))
bagging_error <- sapply(1:B, function(i) {
  mean_votes <- rowMeans(bagging_pred_numeric[, 1:i, drop=FALSE])
  preds <- ifelse(mean_votes > 0.5, 1, 0)
  mean(preds != test_y_numeric)
})

errors_df <- data.table(
  Terms = rep(1:B, 4), 
  Error = c(adaboost_error, bagging_error, rep(stump_error,B), rep(tree_error,B)),
  Method = factor(rep(c("AdaBoost", "Bagging", "Single Stump", "66 Node Tree"), each = B)) # Changed from 5 levels
)
...
ggplot(errors_df, aes(x = Terms, y = Error, color = Method)) +
  ...
  theme_minimal() +
  labs(title = "Ensemble Method Comparison (Real Data)",
       x = "Number of Terms",
       y = "Test Error",
       color = "Method")
