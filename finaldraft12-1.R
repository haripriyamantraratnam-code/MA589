...
#error message for collinearity, dropping Nationality and keeping International 
lda.fit <- lda(y ~ ., data = dat.train)
pred.lda <- predict(lda.fit, newdata = dat.test)
table(Predicted = pred.lda$class, Actual = dat.test$y)
mean(pred.lda$class == dat.test$y)

# AI help for metrics 
library(caret)
library(pROC)

conf_matrix <- table(LDA_Predicted = pred.lda$class, True = dat.test$y)
print(conf_matrix)
metrics <- confusionMatrix(data = pred.lda$class, reference = as.factor(dat.test$y), mode = "everything")
...
multiclass_auc <- multiclass.roc(response = dat.test$y, predictor = predicted_probabilities)
overall_multiclass_auc <- multiclass_auc$auc
cat("Overall Multi-class AUC (Hand and Till): ", overall_multiclass_auc, "\n")

## PC 

train_pca <- prcomp(dat.train[, -which(names(dat.train) == "y")], scale. = TRUE)
test_pca <- predict(train_pca, newdata = dat.test[, -which(names(dat.test) == "y")])
data_pca <- data.frame(PC1 = pca$x[,1], PC2 = pca$x[,2], y = data$y, label=idx)
...
train_pca_scores <- as.data.frame(train_pca$x[, 1:2]) %>%
  mutate(y = dat.train$y, label = "Train")
test_pca_scores <- as.data.frame(test_pca[, 1:2]) %>%
  mutate(y = dat.test$y, label = "Test")
...
# How do I correct for different number of observations in training and testing? 

## QDA 
qda.fit <- qda(y ~ ., data = dat.train)
pred.qda <- predict(qda.fit, newdata = dat.test)
table(Predicted = pred.qda$class, Actual = dat.test$y)
mean(pred.qda$class == dat.test$y)

# AI help for metrics 
library(caret)
library(pROC)

conf_matrix <- table(QDA_Predicted = pred.qda$class, True = dat.test$y)
print(conf_matrix)
metrics <- confusionMatrix(data = pred.qda$class, reference = as.factor(dat.test$y), mode = "everything")
...
data_lasso$y <- as.factor(data_lasso$y)
all_possible_levels <- levels(data_lasso$y) 
...
logit_model <- nnet::multinom(y ~., data = dat.train.lasso)
summary(logit_model)

logit_pred <- logit_model %>% predict(dat.test.lasso)
head(logit_pred)
